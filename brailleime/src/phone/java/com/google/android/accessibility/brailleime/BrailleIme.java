...
import com.google.android.accessibility.utils.brailleime.TalkBackForBrailleIme.ServiceStatus;
...
public class BrailleIme extends InputMethodService {
  ...
  public static class ImeConnection {
    ...
    public ImeConnection(
        InputConnection inputConnection, EditorInfo editorInfo, boolean shouldAnnounceCharacter) {
      this.inputConnection = inputConnection;
      this.editorInfo = editorInfo;
      this.shouldAnnounceCharacter = shouldAnnounceCharacter;
    }
  }
  ...
  boolean switchContractedMode() {
    ...
    brailleImeAnalytics.logContractedToggle(contractedModeEnabled);
    return true;
  }
  ...
  private void createEditBuffer() {
    ...
    if (Utils.isPasswordField(getCurrentInputEditorInfo())) {
      // Force switch to UEB with non-contracted if this is a password-entering field. Do this
      // regardless of language - assume passwords use the Latin alphabet.
      code = Code.UEB;
  ...
  private ImeConnection getImeConnection() {
    return new ImeConnection(
        ...
        talkBackForBrailleIme.shouldAnnounceCharacter());
  }
